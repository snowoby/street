...
func (sq *SeriesQuery) QueryOwner() *ProfileQuery {
...
func (sq *SeriesQuery) QueryParticipant() *ProfileQuery {
	query := &ProfileQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (sq *SeriesQuery) Clone() *SeriesQuery {
	...
	return &SeriesQuery{
		...
		withParticipant: sq.withParticipant.Clone(),
...
func (sq *SeriesQuery) WithOwner(opts ...func(*ProfileQuery)) *SeriesQuery {
	query := &ProfileQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SeriesQuery) WithParticipant(opts ...func(*ProfileQuery)) *SeriesQuery {
	query := &ProfileQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withParticipant = query
	return sq
}
...
func (sq *SeriesQuery) sqlAll(ctx context.Context) ([]*Series, error) {
	...
	if query := sq.withEpisodes; query != nil {
		...
		query.Where(predicate.Episode(func(s *sql.Selector) {
			s.Where(sql.InValues(series.EpisodesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "series_episodes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "series_episodes" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := sq.withParticipant; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Series, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Participant = []*Profile{}
		}
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Series)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(series.ParticipantPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "participant": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "participant" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Participant = append(nodes[i].Edges.Participant, n)
			}
		}
	}
