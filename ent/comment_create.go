...
func (cc *CommentCreate) SetSid(s schema.ID) *CommentCreate {
	cc.mutation.SetSid(s)
	return cc
}

// SetNillableSid sets the "sid" field if the given value is not nil.
func (cc *CommentCreate) SetNillableSid(s *schema.ID) *CommentCreate {
	if s != nil {
		cc.SetSid(*s)
	}
	return cc
...
func (cc *CommentCreate) SetEpisode(e *Episode) *CommentCreate {
	return cc.SetEpisodeID(e.ID)
}
...
func (cc *CommentCreate) defaults() {
	if _, ok := cc.mutation.Sid(); !ok {
		v := comment.DefaultSid()
		cc.mutation.SetSid(v)
	}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.EpisodeIDs(); len(nodes) > 0 {
		...
		_node.episode_comments = &nodes[0]
