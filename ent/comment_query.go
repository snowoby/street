...
func (cq *CommentQuery) QueryEpisode() *EpisodeQuery {
...
func (cq *CommentQuery) QueryAuthor() *ProfileQuery {
	query := &ProfileQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithEpisode(opts ...func(*EpisodeQuery)) *CommentQuery {
	query := &EpisodeQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withEpisode = query
	return cq
}

// WithAuthor tells the query-builder to eager-load the nodes that are connected to
// the "author" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithAuthor(opts ...func(*ProfileQuery)) *CommentQuery {
	query := &ProfileQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context) ([]*Comment, error) {
	...
	if query := cq.withEpisode; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Comment)
		for i := range nodes {
			if nodes[i].episode_comments == nil {
				continue
			}
			fk := *nodes[i].episode_comments
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "episode_comments" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Episode = n
			}
		}
	}
	...
	if query := cq.withAuthor; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Comment)
		for i := range nodes {
			if nodes[i].profile_commenter == nil {
				continue
			}
			fk := *nodes[i].profile_commenter
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "profile_commenter" returned %v`, n.ID)
			}
			for i := range nodes {
